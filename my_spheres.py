...
knifeVelocity = 5                 #velocity of knife; better not be variated for a while
                                  #knife moves from the start, delay hasn't been made yet

def currentVelocity(knifeVelocity, bollean = True):
    if bollean:
        print(bollean)
...
v.eyePosition = Vector3(boxLength/2,-boxWidth*3, boxHeight*0.7)
v.viewDir = Vector3(0, 1, -0.35*boxHeight/boxLength)
v.sceneRadius = boxLength
v.screenSize = Vector2i(1280, 760)

factory = BoxFactory(maxParticles = 5000, maxMass = -1,
        center = (boxLength/2, boxWidth/2, 2*boxHeight),
        extents = (boxLength/2, boxWidth/2, 2*boxHeight),
        vMin = 20.0, vMax = 25.0,
		PSDsizes = Diams, PSDcum = massCum,
        PSDcalculateMass = True, exactDiam = False,
...
O.engines=[
    ...
    factory,
    PyRunner(iterPeriod = 1, command = 'knifeVelocity, factory.numParticles == factory.maxParticles'),
    TranslationEngine(translationAxis = [1, 0, 0],
                      velocity = currentVelocity(knifeVelocity),
                      ids = Knifes[0]),
